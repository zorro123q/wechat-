...
Page({
  ...
  hideIssueCertModal() {
    this.setData({ showIssueCertModal: false });
  },

  onCertLevelChange(e) {
    this.setData({ certLevelIndex: Number(e.detail.value || 0) });
  },

  onCertNameInput(e) {
    this.setData({ certName: String(e.detail.value || '') });
  },

  confirmIssueCertificate() {
    const { selectedUser, certLevelIndex, certName, certLevelOptions } = this.data;
    if (!selectedUser) {
      wx.showToast({ title: '请选择学员', icon: 'none' });
      return;
    }
    const levelMap = [1, 2, 3];
    const level = levelMap[certLevelIndex] || 1;
    const name = String(certName || '').trim();

    wx.showLoading({ title: '发放中...' });
    api.adminIssueCertificate({
      userId: selectedUser._id,
      level,
      certName: name || `${certLevelOptions[certLevelIndex] || '初级'}训机师证书`
    }).then(() => {
      wx.hideLoading();
      wx.showToast({ title: '发放成功' });
      this.hideIssueCertModal();
    }).catch(err => {
      wx.hideLoading();
  ...
  searchOrder(code) {
    ...
    wx.cloud.callFunction({
      ...
      if (r.success && r.order) {
        this.setData({ verifyOrder: r.order });
        return;
      }
      wx.showToast({ title: r.message || '未找到订单', icon: 'none' });
    }).catch(err => {
      wx.hideLoading();
  ...
  goToPointsLog() {
    wx.navigateTo({
      url: '/pages/admin-points-logs/admin-points-logs'
    });
  },
  ...
  goToCategoryManage() {
    wx.navigateTo({
      url: '/pages/admin-categories/admin-categories'
    });
  },
