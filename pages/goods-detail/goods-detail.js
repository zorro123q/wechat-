...
Page({
  ...
  loadGoodsDetail() {
    ...
    api.getGoodsDetail(this.data.goodsId).then(res => {
      ...
      const images = (goods && goods.images) ? goods.images : [];
      const cloudIds = images.filter(s => this.isCloudFileId(s));

      const applyDisplayImages = (urls) => {
        const cover = urls && urls[0] ? urls[0] : '';
        this.setData({
          goods,
          displayImages: urls && urls.length ? urls : images,
          displayCover: cover,
          loading: false
        });
        this.updateFormattedPoints();
      ...
      if (cloudIds.length === 0) {
        applyDisplayImages(images);
        return;
      }

      api.getTempFileURLCached(cloudIds).then(map => {
        const urls = images.map(s => (this.isCloudFileId(s) ? (map[s] || s) : s));
        applyDisplayImages(urls);
      }).catch(() => {
        applyDisplayImages(images);
      });
    }).catch(err => {
