...
function getLevelInfo(points) {
  ...
  for (const level of levels) {
    if (points >= level.minPoints) {
      ...
      if (level.key === 'junior') {
        nextLevel = levels[1];
        progress = Math.min(100, (points / nextLevel.minPoints) * 100);
      } else if (level.key === 'intermediate') {
        nextLevel = levels[0];
        progress = Math.min(100, ((points - level.minPoints) / (nextLevel.minPoints - level.minPoints)) * 100);
      }
      ...
      return {
        ...
        nextLevel,
        progress,
        pointsToNext: nextLevel ? Math.max(0, nextLevel.minPoints - points) : 0
      };
    }
  }
